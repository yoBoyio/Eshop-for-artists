...
class Upload extends Component {
  ...
  handleToggle(value) {
    ...
    if (currentIndex === -1) {
      ...
      this.setState({ selectedGenres: [...this.state.selectedGenres, this.state.genres[value - 1]] })
  ...
  handleAdditionTags(e) {
    if (e.key === "Enter") {
      this.setState(state => ({ tags: [...state.tags, e.target.value] }));
    }
  }
  ...
  render() {
    const { title, price, errors, tags, genres, checked, BPM, switchChecked, filenamemp3, filenamePhoto } = this.state;
    const {
      classes,
        ...
        <form onSubmit={this.handleSubmit}>
          ...
          <List className={classes.root}>
            ...
            {genres.map((genre) =>
              ...
              <ListItem key={genre.id} role={undefined} dense button onClick={() => this.handleToggle(genre.id)}>
                <ListItemIcon>
                  <Checkbox
